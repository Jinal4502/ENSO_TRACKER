...
async function loadRegion(key) {{
  ...
  if (!csvCache[key]) {{
    try {{
      ...
      if (!resp.ok) throw new Error(
        rc.name+" temperature data not yet available — run convert_temperature.py locally and commit the CSV files.");
  ...
  stateRows={{}}; cachedSliderSteps=[]; cmapMin=-10; cmapMax=40;
  currentMode="tracker";
  ...
  // Colorscale bounds: 2nd–98th percentile of all values
  const allVals=[];
