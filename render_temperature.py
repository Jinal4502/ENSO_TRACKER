"""
render_temperature.py
Generates docs/temperature.html — Global Temperature dashboard.
Identical structure to render_precipitation.py; differences:
  • CSV: {region}_temp_monthly_grid.csv  (variable: temp_c, °C)
  • Diverging red-blue colorscale (cold → hot)
  • Y-axis: °C, no fill-to-zero, no rangemode:tozero
...
REGION_CONFIG = {
    "usa": {
        "name": "United States",
        "csv": "data/usa_temp_monthly_grid.csv",
        "center_lat": 39.5, "center_lon": -96.0, "zoom": 2.5,
        "marker_size": 20,
        "all_label": "All USA",
        "subregion_label": "Region",
        "source_html": (
            "GHCN-CAMS 2m air temperature · 1.0° grid · "
            "<a href='https://psl.noaa.gov/data/gridded/data.ghcncams.html'"
            " target='_blank'>NOAA/PSL</a>"
        ),
    },
    "india": {
        "name": "India",
        "csv": "data/india_temp_monthly_grid.csv",
    ...
    "australia": {
        "name": "Australia",
        "csv": "data/australia_temp_monthly_grid.csv",
    ...
    "brazil": {
        "name": "Brazil",
        "csv": "data/brazil_temp_monthly_grid.csv",
    ...
    "east_africa": {
        "name": "East Africa",
        "csv": "data/east_africa_temp_monthly_grid.csv",
    ...
    "china": {
        "name": "China",
        "csv": "data/china_temp_monthly_grid.csv",
    ...
    "global": {
        "name": "Global",
        "csv": "data/global_temp_monthly_grid.csv",
        "center_lat": 20.0, "center_lon": 0.0, "zoom": 0.6,
        "marker_size": 7,
        "all_label": "Global Average",
        "subregion_label": "Region",
        "source_html": (
            "GHCN-CAMS 2m air temperature · 2.5° grid · "
            "<a href='https://psl.noaa.gov/data/gridded/data.ghcncams.html'"
            " target='_blank'>NOAA/PSL</a>"
        ),
    },
...
def render_temperature(output_path: str = "docs/temperature.html") -> None:
...
<style>
  ...
  * {{ box-sizing:border-box; margin:0; padding:0; }}
  ...
  .nav-links a.nav-active {{ color:#fff; background:#e05d44; font-weight:600; }}
  ...
  .btn[data-tooltip]::after {{
    ...
    background:#1c2128; color:#c9d1d9; font-size:.72rem; font-weight:400;
    padding:3px 8px; border-radius:4px; border:1px solid #30363d;
    white-space:nowrap; pointer-events:none;
    opacity:0; transition:opacity 0.05s; z-index:200;
  }}
  .btn[data-tooltip]:hover::after {{ opacity:1; }}
</style>
</head>
<body>
...
<div id="loading" style="text-align:center;padding:3rem;color:var(--muted)">
  Loading temperature data&hellip;
</div>
...
<div id="content">
  ...
  <div class="card">
    <h2>Global Land Temperature Map</h2>
    ...
    <div id="compositeControls" style="display:none">
      <p style="font-size:.78rem;color:var(--muted);margin-bottom:.5rem">
        Average temperature per cell for each ENSO phase. Toggle to compare spatial patterns.
      </p>
  ...
  <div class="card">
    <h2 id="lineTitle">Monthly Temperature</h2>
    <p style="font-size:.78rem;color:var(--muted);margin-bottom:.5rem">
      Area-average monthly 2m temperature.
  ...
  <div class="card">
    ...
    <p style="font-size:.78rem;color:var(--muted);margin-bottom:.5rem">
      Long-term average temperature for each calendar month, split by ENSO phase.
    </p>
    <div id="climDiv"></div>
  </div>
...
// Diverging cold (blue) → hot (red) colorscale
const TEMP_SCALE = [
  [0.00,"#053061"],
  [0.10,"#2166ac"],
  [0.22,"#4393c3"],
...
function makeTrace(vals) {{
  ...
  return {{
    ...
    customdata:fixedStates,
    marker:{{
      size:ms, opacity:0.88,
      color:vals, colorscale:TEMP_SCALE, cmin:cmapMin, cmax:cmapMax,
      colorbar:{{
        title:{{text:"°C",font:{{color:"#fca5a5",size:11}}}},
        tickfont:{{color:"#fca5a5",size:10}},
        bgcolor:"rgba(12,17,23,0.80)",bordercolor:"#450a0a",borderwidth:1,
        len:0.55,thickness:14,x:1.01,xpad:8
      }}
    }},
    hovertemplate:"<b>%{{customdata}}</b><br>%{{lat:.1f}}°, %{{lon:.1f}}°<br>%{{marker.color:.1f}} °C<extra></extra>",
    showlegend:false
  }};
}}
...
document.getElementById("ensoToggle").addEventListener("click", e => {{
  ...
  btn.classList.add("active");
  Plotly.react("mapDiv",[makeTrace(composites[btn.dataset.enso])],
    makeCompositeLayout(btn.dataset.enso),{{responsive:true}});
}});
...
function renderTimeSeries(stateKey) {{
  ...
  for(const r of rows){{
    ...
    mdata[k].sum+=+r.temp_c; mdata[k].cnt++;
  }}
  ...
  const PCT_LINES=[
    {{p:10,val:pct(sorted,10),color:"#60a5fa",dash:"dot"}},
    {{p:50,val:pct(sorted,50),color:"#3fb950",dash:"dash"}},
    {{p:90,val:pct(sorted,90),color:"#f87171",dash:"dot"}},
  ];
  ...
  document.getElementById("lineTitle").textContent =
    "Monthly Temperature — "+(stateKey==="all"?REGION_CONFIG[currentRegion].all_label:stateKey)+" ("+YR0+"–"+YR1+")";

  Plotly.react("lineDiv",[
    {{type:"scatter",mode:"lines",x:msX,y:msY,
      line:{{color:"#f87171",width:1.5}},
      customdata:msENSO,
      hovertemplate:"<b>%{{x|%b %Y}}</b><br>%{{y:.1f}} °C<br>%{{customdata}}<extra></extra>",
      showlegend:false}},
    ...
    yaxis:{{title:"°C",color:DARK.muted,gridcolor:DARK.grid}},
...
function renderClimatology(stateKey) {{
  ...
  for(const r of rows){{
    ...
    byPhase[ph][+r.month-1].s+=+r.temp_c; byPhase[ph][+r.month-1].n++;
  }}
  const label = stateKey==="all" ? REGION_CONFIG[currentRegion].all_label : stateKey;
  ...
  Plotly.react("climDiv",
    ENSO_PHASES.map((ph,i)=>({{
      ...
      y:byPhase[ph].map(d=>d.n?+(d.s/d.n).toFixed(1):null),
      marker:{{color:ENSO_COLORS[i],opacity:0.85}},
      hovertemplate:"<b>%{{x}} — "+ph+"</b><br>%{{y:.1f}} °C avg<extra></extra>"
    }})),
  {{
...
// ── Load global temperature map ───────────────────────────────────────────────
async function loadGlobalMap() {{
  const rc = REGION_CONFIG[MAP_KEY];
  document.getElementById("loading").textContent = "Loading global temperature data…";

  if (!csvCache[MAP_KEY]) {{
    try {{
      const resp = await fetch(rc.csv);
      if (!resp.ok) throw new Error(
        "Global temperature data not yet available — run convert_temperature.py "
        +"locally and commit docs/data/global_temp_monthly_grid.csv to the repo.");
  ...
  cachedSliderSteps=[]; cmapMin=-10; cmapMax=40; currentMode="tracker";
  ...
  for(const r of rows){{
    ...
    if(ci!==undefined) rawMonthly[k].vals[ci]=+r.temp_c;
  }}
  ...
  cmapMin=Math.floor(allVals[Math.floor(allVals.length*0.02)]);
  cmapMax=Math.ceil(allVals[Math.floor(allVals.length*0.98)]);
  ...
  document.getElementById("pageTitle").textContent="Global Land Temperature";
...
async function loadRegionCharts(key) {{
  ...
  if (!csvCache[key]) {{
    try {{
      const resp = await fetch(rc.csv);
      if (!resp.ok) throw new Error(rc.name+" temperature data not yet available — run convert_temperature.py locally and commit the CSV files.");
...
"""
    ...
    print(f"Temperature page written → {output_path}  ({size_kb} KB)")
