...
def render(data: dict, output_path: str = "docs/index.html") -> None:
    ...
    TIP_PLUME    = _tip("Each line is one model's Niño-3.4 anomaly forecast. ● Filled circles = dynamical models: physics-based coupled ocean–atmosphere GCMs that simulate the fluid equations governing climate (e.g. ECMWF, NCEP CFSv2). ○ Open circles = statistical models: data-driven methods trained on historical SST patterns (e.g. CPC CA, LDEO). ◇ Open diamonds = relative models: forecast the Niño-3.4 anomaly expressed relative to the tropical Pacific mean SST rather than a fixed climatological baseline, removing the long-term warming trend (the same concept as RONI). Stars = ensemble averages per type. Use the dropdown to filter.")
